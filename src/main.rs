...
#[fehler::throws(Box<dyn std::error::Error>)] fn main() {
	...
	let Simulation{state: combustion::State{temperature, amounts}, pressure_r, species, ..} = Simulation::new(&system)?;
	let ([viscosity, thermal_conductivity], ref thermal_diffusion_coefficients) = {
		...
		unsafe {
			cantera(pressure_r*combustion::ideal_gas_constant, temperature, mole_proportions.as_ptr(), &mut viscosity, &mut thermal_conductivity, &mut species_len, &mut specie_names, &mut thermal_diffusion_coefficients);
