...
#[fehler::throws(anyhow::Error)] fn main() {
	...
	let stride = 1;
	let len = 1*stride;
	let [mut pressure_r, mut temperature, mut volume] = [pressure_r, temperature, volume].map(|initial| Buffer::new(device, (0..len).map(|_| initial)).unwrap());
	...
	device.submit_and_wait(&[&[&mut pressure_r],&[&mut temperature],&[&mut volume], &box_collect(amounts.iter_mut().map(|n| n)), &box_collect(jacobian.iter_mut().map(|n| n))], stride, len)?;
	let gpu_f = (
