...
#[fehler::throws(Box<dyn std::error::Error>)] fn main() {
	...
	let temperature = Buffer::new(device, (0..len).map(|_| *temperature as f32))?;
	let amounts_buffers = box_collect(amounts.iter().map(|&n| Buffer::new(device, (0..len).map(|_| n as f32)).unwrap()));
	let ref_amounts = box_collect(amounts_buffers.iter().map(|n| n));
	let d_temperature = Buffer::new(device, (0..len).map(|_| f32::NAN))?;
	let d_amounts = box_collect(amounts.iter().map(|_| Buffer::new(device, (0..len).map(|_| f32::NAN)).unwrap()));
	let ref_d_amounts = box_collect(d_amounts.iter().map(|n| n));
	let viscosity = Buffer::new(device, (0..len).map(|_| f32::NAN))?;
	let thermal_conductivity = Buffer::new(device, (0..len).map(|_| f32::NAN))?;
	let mixture_averaged_thermal_diffusion_coefficients = iter::eval(species.len(), |_| Buffer::new(device, (0..len).map(|_| f32::NAN)).unwrap());
