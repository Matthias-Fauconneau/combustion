...
#[fehler::throws(anyhow::Error)] fn main() {
	...
	let stride = 1;
	let len = 1*stride;
	let constants = Buffer::new(device, BufferUsageFlags::UNIFORM_BUFFER, [volume, pressure_r].iter())?;
	...
	device.submit_and_wait(&constants, &[&[&mut temperature], &box_collect(amounts.iter_mut().map(|n| n))/*, &box_collect(jacobian.iter_mut().map(|n| n))*/], stride, len)?;
	let gpu_f = (
