			...
			.zip(cR.iter().zip(forward.iter().zip(reverse.iter()))) {
				...
				fn arrhenius(RateConstant{preexponential_factor, temperature_exponent, activation_temperature}: RateConstant, T: f64) -> f64 {
					if [0.,-1.,1.,2.,4.,-2.].contains(&temperature_exponent) && activation_temperature == 0. {
						...
						let logA = f64::log2(preexponential_factor);
						let βlogT𐊛logA = if temperature_exponent == 0. { logA } else { fma(temperature_exponent, f64::log2(T), logA) };
						let log_arrhenius = if activation_temperature == 0. { βlogT𐊛logA } else { fma(-activation_temperature/LN_2, 1./T, βlogT𐊛logA) };
						f64::exp2(log_arrhenius)
					}
				}
				...
				fn efficiency(model: &ReactionModel, T: f64, concentrations: &[f64], k_inf: f64) -> f64 {
					use ReactionModel::*; match model {
						...
						Falloff{efficiencies, k0, troe} => {
							...
							let Fcent = fma(1.-A, f64::exp2(-T*rcp(LN_2*T3)), fma(A, f64::exp2(-T*rcp(LN_2*T1)), f64::exp2(-1./T * T2/LN_2)));
				...
				let rcpV = 1./volume;
				let pressure = state_vector[1];
				let kT = K*T;
				let total_concentration = pressure/kT;
				...
				let Rr = rcp_equilibrium_constant * product_of_exponentiations(products.iter().copied().zip(concentrations.iter().copied()));
				{
