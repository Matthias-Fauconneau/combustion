...
pub fn check(model: &model::Model, state: &State) {
	...
	let ref _mixture_mass_averaged_thermal_diffusion_coefficients = {
		...
		if false {
			...
			"5.88830303e-04 9.74876528e-04 2.54222365e-04 1.48570420e-04 2.49489253e-04 2.11459327e-04 1.65222080e-04 1.64120812e-04 2.38722194e-04 2.71708310e-04 1.85500267e-04 1.85500267e-04 1.81398673e-04 1.97357482e-04 1.63943507e-04 1.32876379e-04 1.41676796e-04 1.40531852e-04 1.38019881e-04 1.38019881e-04 1.37553058e-04 1.39820441e-04 1.38417244e-04 1.37105272e-04 1.36668970e-04 1.25653611e-04 1.24639892e-04 2.05464735e-04 1.19597783e-04 1.19597783e-04 2.26754242e-04 2.67747169e-04 2.62329021e-04 1.99451789e-04 1.60259336e-04 1.62640124e-04 1.44415354e-04 1.31154960e-04 1.64801742e-04 1.61420566e-04 1.40439564e-04 1.39179409e-04 2.05460769e-04 1.31725863e-04 1.31725863e-04 1.31725863e-04 1.32333100e-04 1.64317769e-04 1.61220023e-04 9.72742481e-05 9.68451144e-05 1.19049476e-04 1.18523749e-04".split(' ').map(|s| s.parse().unwrap()).collect::<Box<[f64]>>()
		}
	};
	let position = |i| cantera_species_names.iter().position(|s| s==species_names[i]).unwrap();
	...
	let _mixture_mass_averaged_thermal_diffusion_coefficients = order(&_mixture_mass_averaged_thermal_diffusion_coefficients);
	let transport::Transport{viscosity, thermal_conductivity, mixture_mass_averaged_thermal_diffusion_coefficients} =
	...
	for (specie, (&a, &b)) in	species_names.iter().zip(binary_thermal_diffusion_coefficients.iter().zip(_binary_thermal_diffusion_coefficients.iter())) {
		assert!(num::relative_error(a, b) < 2e-3, "{}: {} {} {:e} {:e}", specie, a, b, num::abs(a-b), num::relative_error(a, b));
	}
	...
	for (specie, (&a, &b)) in	species_names.iter().zip(
																																		mixture_mass_averaged_thermal_diffusion_coefficients.iter().zip(_mixture_mass_averaged_thermal_diffusion_coefficients.iter())) {
