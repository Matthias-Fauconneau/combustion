...
use {fehler::throws, std::default::default, std::ops::Deref, iter::{Prefix, list, map, DotN}, std::collections::hash_map::{HashMap,Entry::*}, ast::*};
...
#[track_caller] fn check(e: &Expression, f: &mut Block) -> Option<String> {
	{
		let e = e.deref();
...
fn chk(e: Expression, f: &mut Block) -> Expression { check(&e, f); e }
...
fn thermodynamics(thermodynamics: &[NASA7], expression: impl Fn(&[f64; 7], T)->Expression, Ts@T{T,..}: T, f: &mut Block, debug: &str) -> Box<[Expression]> {
	...
	for (temperature_split, ref species) in bucket(thermodynamics.iter().map(|s| ordered_float::OrderedFloat(s.temperature_split))) {
		if temperature_split.is_nan() {
			...
			for e in true_exprs.iter().chain(&*false_exprs) { check(e, f); }
		...
		Falloff{efficiencies, k0, troe} => {
			...
			//f.block(|f|{
				...
				let Fcent = {let Troe{A, T3, T1, T2} = *troe; let T{T,rcp_T,..}=T; ast::sum([
					(T3 > 1e-30).then(|| { let y = 1.-A; if T3<1e30 { y * exp(chk(T/(-T3), f), f) } else { y.into() }}),
					(T1 > 1e-30).then(|| { let y = A; if T1<1e30 { y * exp(chk(T/(-T1), f), f) } else { y.into() }}),
...
fn reaction_rates(reactions: &[Reaction], T: T, C0: &Value, rcp_C0: &Value, exp_Gibbs0_RT: &[Value], concentrations: &[Value], f: &mut Block) -> Box<[Value]> {
	map(reactions.iter().enumerate(), |(_i, Reaction{reactants, products, net, Σnet, rate_constant, model, ..})| {
		...
		let coefficient = if let ReactionModel::Irreversible = model { forward } else {
			...
			let reverse:Expr = le!(f rcp_equilibrium_constant * &product_of_exponentiations(concentrations, products, f));
