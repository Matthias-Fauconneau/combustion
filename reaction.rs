...
fn product_of_exponentiations_<N:Into<i16>>(iter: impl IntoIterator<Item=(&'t Value, N)>, f: &mut Block) -> Value {
	let (num, div) : (Vec::<_>,Vec::<_>) = iter.into_iter().map(|(v,n)| (v,n.into())).filter(|&(_,n)| n!=0).partition(|&(_,n)| n>0);
	let num = num.into_iter().fold(None, |mut p, (v,n)|{ for _ in 0..n { p = Some(match p { Some(p) => le!(f; p*v).clone(), None => v.clone() }); } p });
	let div = div.into_iter().fold(None, |mut p, (v,n)|{ for _ in 0..-n { p = Some(match p { Some(p) => le!(f; p*v).clone(), None => v.clone() }); } p });
	match (num, div) {
		(None, None) => None,
...
fn product_of_exponentiations__<N:iter::IntoExactSizeIterator>(v: &[Value], n: N, f: &mut Block) -> Value where <N as iter::IntoIterator>::Item:Into<i16> {
	use iter::Zip;
	product_of_exponentiations_(v.zip(n), f)
}
fn product_of_exponentiations<I:iter::IntoExactSizeIterator,N:Copy+Into<i16>>(v: &[Value], n: I, f: &mut Block) -> Value where <I as iter::IntoIterator>::Item:Deref<Target=N> {
	use iter::Map;
...
fn thermodynamics(thermodynamics: &[NASA7], expression: impl Fn(&[f64; 7], T)->Expression, Ts@T{T,..}: T, f: &mut Block, debug: &str) -> Box<[Expression]> {
...
fn efficiency(efficiencies: &[f64], concentrations: &[Value], f: &mut Block) -> Expression {
	use iter::Zip;
	let efficiencies = list(efficiencies.zip(concentrations).into_iter().map(|(&e, C)| if e==0. { None } else if e==1. { Some(C.clone()) } else { Some(le!(f e*C)) }).filter_map(|x| x));
...
fn forward_rate_constant(model: &ReactionModel, k_inf: &RateConstant, T: T, concentrations: &[Value], f: &mut Block) -> Ratio {
	use ReactionModel::*; match model {
		...
		PressureModification{efficiencies, k0} => {
			...
			//f.block(|f|{
				let C_k0 = l!(f; efficiency * k0.unwrap());
				let k_inf = l!(f; k_inf.unwrap());
		...
		Falloff{efficiencies, k0, troe} => {
			...
			let k_inf = arrhenius(k_inf, T, f).unwrap();
			let ref k_inf = (|e:Expression|->Expression{ if let Some(x) = e.f64() { x.into() } else { l!(f e) }})(k_inf);
			//f.block(|f|{
				let Pr = l!(f; efficiency * k0 / k_inf);
				...
				let ref lnFcent: Expr = if let Some(x) = Fcent.f64() { x.into() } else { l!(f ln(1./2., Fcent, f)) }; // 0.1-0.7 => e-3
...
fn reaction_rates(reactions: &[Reaction], T: T, C0: &Value, rcp_C0: &Value, exp_Gibbs0_RT: &[Value], concentrations: &[Value], f: &mut Block) -> Box<[Value]> {
	map(reactions.iter().enumerate(), |(_i, Reaction{reactants, products, net, Σnet, rate_constant, model, ..})| {
		...
		let ref coefficient = if let ReactionModel::Irreversible = model { forward } else {
			...
			let reverse:Value = le!(f rcp_equilibrium_constant * &product_of_exponentiations(concentrations, products, f));
...
pub fn rates(species: &[NASA7], reactions: &[Reaction]) -> Function {
	...
	let Gibbs0_RT = thermodynamics(&species[0..active], Gibbs_RT, Ts, f, "Gibbs0_RT");
	...
	use iter::Dot;
	let energy_rate_RT : Expression = rates.dot(&*enthalpy_RT);
	let Cp : Expression = thermodynamics(species, molar_heat_capacity_at_constant_pressure_R, Ts, f, "molar_heat_capacity_at_CP_R").dot(concentrations);
