...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	pub fn compile(f: &ast::Function) -> impl Fn(&[f32]) -> Box<[f32]> + '_ {
	...
	if true {
		...
		let active_amounts = map(&amounts[0..amounts.len()-1], |&v| v as _);
		let_!{ [dtT_T, rates @ ..] = &*rates(&iter::box_([*pressure_R as f32, total_amount as f32, *T as f32].iter().chain(&*active_amounts).copied())) => {
